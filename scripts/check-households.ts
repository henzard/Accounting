...
// Run with: npx ts-node scripts/check-households.ts

import { initializeApp } from 'firebase/app';
import { getFirestore, collection, query, where, getDocs, doc, getDoc } from 'firebase/firestore';
import { firebaseConfig } from '../src/infrastructure/firebase/config';
...
async function getHouseholdMembers(householdId: string): Promise<string[]> {
  try {
    const householdDoc = await getDoc(doc(db, 'households', householdId));
    if (!householdDoc.exists()) {
      return [];
    }
    return householdDoc.data().member_ids || [];
  } catch (error) {
    console.error(`Error getting household ${householdId}:`, error);
    return [];
  }
}

async function getEmailForUserId(userId: string): Promise<string> {
  try {
...
async function checkHouseholds() {
  ...
  for (const householdDoc of householdsSnapshot.docs) {
    ...
    const memberEmails = await Promise.all(
      memberIds.map((memberId: string) => getEmailForUserId(memberId))
    );
  ...
  if (!krugerHousehold) {
    ...
    const hasHenzard = krugerHousehold.member_ids.includes(henzard?.id || '');
    const hasAlicia = krugerHousehold.member_ids.includes(alicia?.id || '');
  ...
  if (!hetzelHousehold) {
    ...
    const hasHenzard = hetzelHousehold.member_ids.includes(henzard?.id || '');
    const hasAlicia = hetzelHousehold.member_ids.includes(alicia?.id || '');
